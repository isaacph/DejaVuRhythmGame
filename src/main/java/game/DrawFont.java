 ...
 * https://github.com/isaacph/lostandfoundggj/blob/master/src/main/java/isaacais/Font.java
 */
public class DrawFont {
    ...
    public DrawFont(String source, int size, int bmpWidth, int bmpHeight) {
        ...
        int vertex = ShaderUtil.createShader("textv.glsl", GL_VERTEX_SHADER);
        int fragment = ShaderUtil.createShader("textf.glsl", GL_FRAGMENT_SHADER);
        shader = glCreateProgram();
        glAttachShader(shader, vertex);
        glAttachShader(shader, fragment);
        glBindAttribLocation(shader, ShaderUtil.Attribute.POSITION.position, "position");
        glBindAttribLocation(shader, ShaderUtil.Attribute.TEXTURE.position, "tex");
        glLinkProgram(shader);
        ShaderUtil.checkLinking(shader);
        glUseProgram(shader);
        ShaderUtil.checkGLError("Error using font program " + shader + " path: " + source);
        ...
        try {
            ...
            try (MemoryStack stack = MemoryStack.stackPush())
            {
                ...
                ShaderUtil.checkGLError("Texture " + source);
                this.texture = tex;
            }
...
//                vertices.flip();
        ...
        ShaderUtil.checkGLError("Font init " + source);
    }
